...
eficall! {pub fn reinstall_protocol_interface (
  ...
  new_interface: *mut c_void) -> r_efi::efi::Status {

  if protocol.is_null() ||
    old_interface.is_null() ||
    new_interface.is_null()
  {
    return r_efi::efi::Status::INVALID_PARAMETER;
  }
...
eficall! {pub fn locate_handle (
  ...
  handle_buffer: *mut r_efi::efi::Handle) -> r_efi::efi::Status {

  let search_result = match search_type {
    r_efi::efi::ALL_HANDLES => {
      PROTOCOL_DB.locate_handles(None)
    },
  ...
  match search_result {
    ...
    Ok(mut list) => {
      ...
      unsafe {*buffer_size = list.len() * size_of::<r_efi::efi::Handle>();}
...
eficall! {pub fn open_protocol (
  ...
  attributes: u32) -> r_efi::efi::Status {
  ...
  let status = match PROTOCOL_DB.add_protocol_usage(handle, unsafe {*protocol}, agent_handle, controller_handle, attributes) {
...
eficall! {pub fn close_protocol (
  ...
  protocol: *mut r_efi::efi::Guid,
  agent_handle: r_efi::efi::Handle,
  controller_handle: r_efi::efi::Handle) -> r_efi::efi::Status {
...
eficall! {pub fn open_protocol_information (
  ...
  entry_count: *mut usize) -> r_efi::efi::Status {
  ...
  match allocate_pool(
    ...
    buffer_size,
    entry_buffer as *mut *mut c_void) {
    r_efi::efi::Status::SUCCESS => (),
    err => return err
  };
