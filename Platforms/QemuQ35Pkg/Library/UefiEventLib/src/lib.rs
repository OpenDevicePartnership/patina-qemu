...
impl SpinLockedEventDb {
    ...
    pub fn event_notification_iter(
        &'static self,
        tpl_level: r_efi::efi::Tpl,
    ) -> impl Iterator<Item = EventNotification> {
...
mod tests {
    ...
    fn signalled_event_iterator_should_return_next_events_in_tpl_order() {
        ...
        for (event_notification, expected_event) in SPIN_LOCKED_EVENT_DB
            .event_notification_iter(TPL_CALLBACK)
        ...
        for (event_notification, expected_event) in SPIN_LOCKED_EVENT_DB
            .event_notification_iter(TPL_APPLICATION)
    ...
    fn signalling_a_notify_wait_event_should_not_queue_it() {
        ...
        let callback_evt1 = SPIN_LOCKED_EVENT_DB
            .create_event(EVT_NOTIFY_WAIT, TPL_CALLBACK, Some(test_notify_function), None, None)
            .unwrap();
    ...
    fn set_timer_on_event_should_set_timer_on_event() {
        ...
        let event = SPIN_LOCKED_EVENT_DB
            .create_event(EVT_NOTIFY_SIGNAL, TPL_NOTIFY, Some(test_notify_function), None, None)
            .unwrap();
