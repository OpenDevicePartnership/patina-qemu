...
//!
//! pe32_relocate_image(0x04158000, &mut relocated_image, &Vec::new()).unwrap();
//! ```
//!
//! ## License
//!
  ...
  while offset < block.len() {
    ...
    while offset < block_start + block_header.block_size as usize {
      relocations.push(Relocation {
        type_and_offset: block.gread_with(&mut offset, scroll::LE).map_err(|_| Pe32Error::RelocationError)?,
        value: 0,
      });
    }
...
/// pe32_relocate_image(0x04158000, &mut relocated_image, &Vec::new()).unwrap();
/// ```
pub fn pe32_relocate_image(
  destination: usize,
  image: &mut [u8],
  prev_reloc_blocks: &Vec<RelocationBlock>,
) -> Result<Vec<RelocationBlock>, Pe32Error> {
          ...
          match fixup_type {
            ...
            0x0A => {
              ...
              if !prev_reloc_blocks.is_empty() {
                if prev_reloc_blocks[block_idx].relocations[reloc_idx].value != fixup_value {
                  continue;
                }
              }

              fixup_value = fixup_value.wrapping_add(adjustment);
              reloc.value = fixup_value;
...
mod tests {
  ...
  fn pe32_relocate_image_should_work_multiple_times() {
    ...
    pe32_relocate_image(0x80000415, &mut reclocated_image_copy, &Vec::new()).unwrap();

    assert_ne!(relocated_image, reclocated_image_copy);

    pe32_relocate_image(0x04158000, &mut reclocated_image_copy, &Vec::new()).unwrap();
