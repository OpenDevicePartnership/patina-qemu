# Summary

[Introduction](introduction.md)
[Rust Platform Integration](rust_platform_integration.md)

# Platforms

- [QEMU Q35](platforms/q35.md)
- [QEMU ARM Virt](platforms/armvirt.md)

# Building and Running

- [Building the Firmware](building/building.md)
- [Rapid Patina Iteration](building/rapid_iteration.md)

# Debugging

- [WinDbg + QEMU + Patina UEFI](debugging/windbg_uefi.md)
- [WinDbg + QEMU + Windows OS](debugging/windbg_windows.md)

# Testing

- [Regression Testing](testing/regression_testing.md)
- [Self-Certification Testing](testing/self_certification_test.md)

# Performance

- [Collecting Performance Results](performance/results.md)
- [Patina DXE Core Binary Size](performance/binary_size.md)
- [Sample Q35 Performance Report](performance/q35_report_sample.md)

# Trusted Platform Module (TPM)

- [TpmShellApp on QEMU Platforms](tpm/tpm_shell_app.md)
- [TPM on ArmVirt](tpm/tpm_armvirt.md)
- [TPM on Q35](tpm/tpm_q35.md)
